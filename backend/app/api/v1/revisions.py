...
from app.schemas.revisions import RevisionDiffItem, RevisionListResponse, RevisionPublishRequest, RevisionPublishResponse
...
async def publish_revision(
    vehicle_id: UUID,
    revision_id: UUID,
    payload: RevisionPublishRequest = RevisionPublishRequest(),
    db: AsyncSession = Depends(get_db),
    user: UserContext = Depends(get_current_user),
) -> RevisionPublishResponse:
    published_by = user.username
    return await RevisionService(db).publish(
        vehicle_id, revision_id, published_by, label=payload.label
    )
